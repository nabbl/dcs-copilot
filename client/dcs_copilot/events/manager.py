...
from dcs_copilot_protocol import AircraftEvent

from dcs_copilot.rules.base import RuleTransition, RuleTransitionType
...
class EventManager:
    def __init__(
        ...
        *,
        ...
        self._history: deque[ManagedAircraftEvent] = deque(maxlen=history_size)
        self._callbacks: list[Callable[[ManagedAircraftEvent], None]] = []
    ...
    @property
    def history(self) -> tuple[ManagedAircraftEvent, ...]:
        return tuple(self._history)

    def add_callback(self, callback: Callable[[ManagedAircraftEvent], None]) -> None:
    ...
    def handle_transition(self, transition: RuleTransition) -> None:
        ...
        managed = ManagedAircraftEvent(event, transition.timestamp, publish, speak)
